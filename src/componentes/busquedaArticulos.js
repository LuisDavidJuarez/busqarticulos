...
export default function Articulos() {
    ...
    const IniciarValores = () => {
        ...
        setGetway('192.168.13.140');
        peticionGetGatewayData('192.168.13.140');
        console.log("GatewayData: ", GatewayData);

        if (GatewayData.length !== 0) {
            GatewayData.map((data, idx) => {
                return (
                    vSucursal = data.Sucursal
            )
        })
...
const peticionGetGatewayData = async (Gateway) => {
